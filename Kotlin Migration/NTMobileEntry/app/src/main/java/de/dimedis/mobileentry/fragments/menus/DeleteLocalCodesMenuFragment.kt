...
class DeleteLocalCodesMenuFragment : ProgressLoaderFragment(),
    LoaderManager.LoaderCallbacks<Cursor?> {
    var binding: FragmentMenuDeleteLocalCodesBinding? = null
    var mStatusManager: StatusManager? = null

    class Builder {
        fun build(): DeleteLocalCodesMenuFragment {
            return DeleteLocalCodesMenuFragment()
        }
    }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        binding!!.title.text = getLocalizedString(R.string.DELETE_CODES_TITLE)
        binding!!.label.text = getLocalizedString(R.string.DELETE_CODES_INFO_TEXT)
    ...
    override fun onCreateLoader(id: Int, args: Bundle?): Loader<Cursor?> {
        return CursorLoader(
            requireContext(), DataContentProvider.TICKETS_URI, arrayOf("COUNT(*)"),
            null, null, null)
    }
    ...
    override fun onLoadFinished(loader: Loader<Cursor?>, cursor: Cursor?) {
        ...
        if (codeCount > 0) {
            if (!mStatusManager!!.isOnline()) {
                binding!!.label.text = getLocalizedString(R.string.DELETE_CODES_TITLE)
            ...
            binding!!.codesInfo.text =
                getLocalizedString(R.string.DELETE_CODES_CODES_ON_DEVICE_LABEL) + " " + codeCount
        } else {
            binding!!.label.text = getLocalizedString(R.string.DELETE_CODES_NOCODES_TEXT)
            binding!!.codesInfo.text =
                getLocalizedString(R.string.DELETE_CODES_CODES_ON_DEVICE_LABEL) + " " + codeCount
            binding!!.sendCodes.visibility = View.GONE
        }
    ...
    fun onEvent(event: DataBaseUtil.DeleteTicketsEvent?) {
